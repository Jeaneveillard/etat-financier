...
export function showHelpIndex(container) {
...
export function showHelpSection(container, id) {
...
export function bindHelpNav(container) {
...
export function initHelpSystem() {
  ...
  // Intercept clicks on help buttons
  document.addEventListener('click', (e) => {
    // Find if the click is on or inside a help button
    const helpTarget = e.target.closest('.btn-help-inline');
    
    if (helpTarget) {
      e.preventDefault();
      e.stopPropagation(); // Prevent triggering accordions or form submits
      
      // Get help text
      const helpText = helpTarget.getAttribute('data-help');
      if (helpText) {
        content.innerHTML = helpText;
        modal.classList.add('help-modal--visible');
      }
      
      // Optional: tactile feedback on supported mobile devices
